...
### Apps
- `compose.apps.yml`
- Service `app` mặc định build từ `services/s3proxy`.
...
`Internet -> Cloudflare Edge -> cloudflared -> caddy -> s3proxy/ops service`

## 6) Caddy labels và routing

Routing dựa labels trong compose:

- S3Proxy: `${PROJECT_NAME}.${DOMAIN}` (+ alias `main.${DOMAIN}`, `${DOMAIN}`)
...
- Kiểm tra endpoint metrics: `/metrics` (có prefix metric `s3proxy_`).
